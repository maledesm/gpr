...
if __name__ == "__main__":
    print(f"escala          a = {A_MEEP} m/u.MEEP")
    print(f"banda           {F_MIN/1e9:.1f} a {F_MAX/1e9:.1f} GHz "
          f"= {f_meep(F_MIN):.3f} a {f_meep(F_MAX):.3f} u.MEEP")
    print(f"resolucion      {RESOLUCION} celdas/u = {A_MEEP/RESOLUCION*1e3:.1f} mm")
    print(f"bocina          apertura {APERTURA*100:.1f} cm, largo "
          f"{LARGO_TOTAL*100:.1f} cm, bocas a {SEPARACION_BOCAS*100:.1f} cm "
          f"(centros a {SEP_ANTENAS*100:.1f} cm)")
    print(f"placa           {PLACA_ANCHO*100:.1f} cm a {DIST_PLACA*100:.0f} cm")
    print(f"tau cables      {tau_cables()*1e9:.3f} ns")
    print(f"tau interno     {TAU_INTERNO*1e9:.3f} ns")
    print(f"d_offset        {d_offset():.3f} m")
    print(f"placa + offset  {DIST_PLACA + d_offset():.3f} m aparentes")
    print(f"acopl. directo  {SEP_ANTENAS/2 + d_offset():.3f} m aparentes")
