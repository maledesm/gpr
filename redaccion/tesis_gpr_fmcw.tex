...
distancia estimada y el alcance no ambiguo, y se deja planteada la
caracterización comparativa entre los cables originales y un juego
nuevo, más corto, todavía pendiente de fabricar y medir.
...
Con los cables originales del banco ($L_\text{TX} = L_\text{RX} =
3\,\text{m}$, $L_\text{LO} = 0{,}5\,\text{m}$, de modo que
$D = 5{,}5\,\text{m}$) y $\text{VF} = 0{,}66$ (polietileno sólido,
Tabla~\ref{tab:vf_cables}):

\begin{equation*}
  d_\text{offset} = \frac{5{,}5}{2 \times 0{,}66} \approx 4{,}17\,\text{m}.
\end{equation*}
...
\begin{table}[H]
  ...
  \caption{Frecuencia de batido esperada para un blanco a
    ...
    ($D = 5{,}5\,\text{m}$, $\text{VF} = 0{,}66$). $d_\text{offset}
    = 4{,}17\,\text{m}$ es constante; lo que cambia con $T_c$ es a
    cuántos Hz equivale.}
  ...
  \begin{tabular}{cccc}
    ...
    $T_c$ & $f^*(d=1\,\text{m})$ & $f_\text{offset}$
      & $f^*_\text{medido}$ \\
    \midrule
    $40\,\text{ms}$  &  $333\,\text{Hz}$ & $1389\,\text{Hz}$ & $1722\,\text{Hz}$ \\
    $80\,\text{ms}$  &  $167\,\text{Hz}$ &  $694\,\text{Hz}$ &  $861\,\text{Hz}$ \\
    $160\,\text{ms}$ &   $83\,\text{Hz}$ &  $347\,\text{Hz}$ &  $430\,\text{Hz}$ \\
    \bottomrule
  \end{tabular}
\end{table}

La fila de $T_c = 80\,\text{ms}$ es la que se comparó contra el banco:
el modelo predice $861\,\text{Hz}$ para un blanco a $1\,\text{m}$, y lo
observado en la medición superaba los $800\,\text{Hz}$ —compatible
dentro de la incertidumbre de $L_\text{LO}$ y $\text{VF}$ reales, que
todavía no se midieron por separado (Sección~\ref{sec:cables_plantilla}
propone cómo hacerlo con el VNA en vez de asumirlos).
...
Se fabricará un segundo juego de cables, de $30\,\text{cm}$ cada uno en
RG-213, para reemplazar los originales ($3\,\text{m}$ por antena). Con
$D_\text{nuevo} = 2\times0{,}30 - L_\text{LO,nuevo}$, el offset de
distancia se reduce drásticamente frente a los $4{,}17\,\text{m}$
actuales; la Tabla~\ref{tab:cables_nuevo_offset} lo evalúa para
algunas longitudes candidatas del cable al LO (a definir cuando se
arme el juego nuevo completo).

\begin{table}[H]
  \centering
  \caption{Offset de distancia esperado con los cables nuevos
    ($L_\text{TX} = L_\text{RX} = 30\,\text{cm}$, $\text{VF} = 0{,}66$
    nominal) según la longitud que se elija para el cable al LO.}
  \label{tab:cables_nuevo_offset}
  \begin{tabular}{cccc}
    \toprule
    $L_\text{LO,nuevo}$ & $D_\text{nuevo}$ & $d_\text{offset}$ \\
    \midrule
    $0\,\text{cm}$   & $60\,\text{cm}$  & $45{,}5\,\text{cm}$ \\
    $15\,\text{cm}$  & $45\,\text{cm}$  & $34{,}1\,\text{cm}$ \\
    $30\,\text{cm}$  & $30\,\text{cm}$  & $22{,}7\,\text{cm}$ \\
    $50\,\text{cm}$  & $10\,\text{cm}$  & $7{,}6\,\text{cm}$ \\
    \bottomrule
  \end{tabular}
\end{table}

\textbf{Ojo con el signo.} Si $L_\text{LO,nuevo}$ resultara mayor que
...
\begin{enumerate}
  \item \textbf{Pérdida de inserción ($S_{21}$) con el VNA.} Medir
    $S_{21}$ en dB de $900\,\text{MHz}$ a $2\,\text{GHz}$ para el juego
    de cables original y para el nuevo, superpuestos en el mismo
    gráfico (plantilla en la Figura~\ref{fig:cables_s21_template}).
  \item \textbf{Retardo de grupo con el VNA.} Medir la fase de $S_{21}$
    (o el retardo de grupo directo, si el VNA lo entrega) para extraer
    el $\text{VF}$ real de cada cable y compararlo contra el nominal de
    la Tabla~\ref{tab:vf_cables} (plantilla en la
    Figura~\ref{fig:cables_delay_template}).
  \item \textbf{Calibración de un punto en el radar.} Repetir el
  ...
  \item \textbf{Espectro de batido, mismo blanco, los dos juegos de
    cables.} Con un blanco fijo y la misma distancia, capturar el
...
\begin{figure}[H]
  \centering
  % TODO: reemplazar por la captura del VNA en modo S21, 900 MHz a
  % 2 GHz, con las dos curvas (cable original y cable nuevo)
  % superpuestas en el mismo eje.
  % \includegraphics[width=0.9\linewidth]{figuras/cables/s21_original_vs_nuevo.png}
  \fbox{\parbox{0.9\linewidth}{\centering\vspace{3cm}
    \textit{Pendiente: $S_{21}$ [dB] vs.\ frecuencia [MHz],
    $900$--$2000\,\text{MHz}$, cable original y cable nuevo
    superpuestos.}\vspace{3cm}}}
  \caption{Pérdida de inserción medida con el VNA, cable original
    ($3\,\text{m}$) contra cable nuevo (RG-213, $30\,\text{cm}$).}
  \label{fig:cables_s21_template}
\end{figure}

\subsection{Resultados: retardo de grupo y VF real (VNA)}

\begin{figure}[H]
  \centering
  % TODO: reemplazar por la captura del VNA en modo fase de S21 o
  % retardo de grupo, mismo rango de frecuencias, cable original y
  % nuevo superpuestos. Del retardo total y la longitud física del
  % cable se despeja VF = (longitud / c) / retardo medido.
  % \includegraphics[width=0.9\linewidth]{figuras/cables/retardo_original_vs_nuevo.png}
  \fbox{\parbox{0.9\linewidth}{\centering\vspace{3cm}
    \textit{Pendiente: retardo de grupo [ns] vs.\ frecuencia [MHz],
    cable original y cable nuevo superpuestos, con el VF despejado de
    cada uno indicado en la leyenda.}\vspace{3cm}}}
  \caption{Retardo de grupo medido con el VNA y factor de velocidad
    real despejado, cable original contra cable nuevo.}
  \label{fig:cables_delay_template}
\end{figure}
...
\begin{table}[H]
  \centering
  \caption{Resumen comparativo entre el valor teórico (predicho por
    las ecuaciones de este capítulo, a partir de longitudes medidas
    con cinta métrica y $\text{VF}$ nominal de catálogo) y el valor
    medido (VNA y calibración en el radar), para ambos juegos de
    cables. Completar al finalizar la caracterización.}
  \label{tab:cables_teoria_practica}
  \begin{tabular}{lcccc}
    \toprule
    & \multicolumn{2}{c}{\textbf{Cable original}}
    & \multicolumn{2}{c}{\textbf{Cable nuevo (RG-213)}} \\
    \cmidrule(lr){2-3}\cmidrule(lr){4-5}
    & Teoría & Medido & Teoría & Medido \\
    \midrule
    VF                                    & $0{,}66$\textsuperscript{*} & --- & $0{,}66$\textsuperscript{*} & --- \\
    $D$ {[}m{]}                           & $5{,}50$ & --- & \textit{(a def.)} & --- \\
    $d_\text{offset}$ {[}m{]}              & $4{,}17$ & --- & \textit{(a def.)} & --- \\
    Atenuación @ $1{,}5\,\text{GHz}$ {[}dB{]} & --- & --- & --- & --- \\
    \bottomrule
  \end{tabular}

  \vspace{4pt}
  \footnotesize\textsuperscript{*}Valor nominal de catálogo
  (Tabla~\ref{tab:vf_cables}); reemplazar por el VF despejado del
  retardo de grupo medido con el VNA (Sección anterior) en cuanto esté
  disponible.
\end{table}
