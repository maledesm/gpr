...
\begin{itemize}
  \item \textbf{Camino del LO} (corto): splitter $\to$ cable de
    longitud $L_\text{LO}$ $\to$ entrada LO del mezclador.
...
\begin{figure}[H]
  ...
  \end{tikzpicture}
  \caption{Los dos caminos que llegan al mezclador. El del LO es
    corto y fijo; el de RF incluye los cables a las antenas más el
...
$L_\text{TX} = 3\,\text{m}$ y $L_\text{RX} = 2{,}6\,\text{m}$, con
$L_\text{LO} = 0{,}5\,\text{m}$, de modo que $D = 5{,}1\,\text{m}$---
y $\text{VF} = 0{,}66$ (polietileno sólido,
Tabla~\ref{tab:vf_cables}):

\begin{equation*}
  d_\text{offset} = \frac{5{,}1}{2 \times 0{,}66} \approx 3{,}86\,\text{m}.
\end{equation*}

Nótese que la ecuación~\eqref{eq:D_neto} solo pide la \emph{suma}
$L_\text{TX} + L_\text{RX}$: que los dos cables de antena sean de
distinto largo no agrega ningún término. El desbalance que importa es
el que hay entre el camino de RF y el del LO, no el que hay entre TX y
RX.
...
\begin{table}[H]
  ...
  \caption{Frecuencia de batido esperada para un blanco a
    ...
    ($D = 5{,}1\,\text{m}$, $\text{VF} = 0{,}66$). $d_\text{offset}
    = 3{,}86\,\text{m}$ es constante; lo que cambia con $T_c$ es a
    cuántos Hz equivale.}
  ...
  \begin{tabular}{cccc}
    ...
    $T_c$ & $f^*(d=1\,\text{m})$ & $f_\text{offset}$
      & $f^*_\text{medido}$ \\
    \midrule
    $40\,\text{ms}$  &  $334\,\text{Hz}$ & $1289\,\text{Hz}$ & $1622\,\text{Hz}$ \\
    $80\,\text{ms}$  &  $167\,\text{Hz}$ &  $644\,\text{Hz}$ &  $811\,\text{Hz}$ \\
    $160\,\text{ms}$ &   $83\,\text{Hz}$ &  $322\,\text{Hz}$ &  $406\,\text{Hz}$ \\
    \bottomrule
  \end{tabular}
\end{table}

La fila de $T_c = 80\,\text{ms}$ es la que se comparó contra el banco:
el modelo predice $811\,\text{Hz}$ para un blanco a $1\,\text{m}$, y lo
observado en la medición superaba los $800\,\text{Hz}$. La coincidencia
es mejor de lo que el método merece ---$L_\text{LO}$ y el $\text{VF}$
real siguen siendo valores supuestos, no medidos--- pero deja el orden
de magnitud fuera de discusión: el aporte de los cables es varias veces
el del blanco.
...
El retardo de los cables puede anularse por diseño, igualando
$L_\text{LO}$ a $L_\text{TX} + L_\text{RX}$ para que $D = 0$ en la
ecuación~\eqref{eq:D_neto}. La \textbf{atenuación} de los cables, en
cambio, no se cancela nunca: cada tramo atenúa la señal que lo atraviesa
sin importar cuánto midan los otros. Un coaxil atenúa más cuanto:
...
distancia cae frente a los $3{,}86\,\text{m}$ del juego original; la
Tabla~\ref{tab:cables_nuevo_offset} lo evalúa según qué se haga con el
cable al LO, que es el único grado de libertad que queda.
...
\begin{table}[H]
  ...
  \begin{tabular}{ccc}
    ...
    \midrule
    $0{,}0\,\text{m}$ & $2{,}00\,\text{m}$ & $1{,}52\,\text{m}$ \\
...
\textbf{Ojo con el signo.} Si $L_\text{LO}$ resultara mayor que
$L_\text{TX} + L_\text{RX}$, $D$ se vuelve negativo y con él
$d_\text{offset}$: un blanco aparecería más \emph{cerca} de lo que
está en realidad, en vez de más lejos. Conviene medir $L_\text{LO}$ y
recortarlo junto con los cables de antena, no dejarlo como estaba.
...
baja el offset de $3{,}86\,\text{m}$ a $1{,}14\,\text{m}$. Sigue siendo
ocho veces la resolución de $14{,}4\,\text{cm}$, así que \textbf{acortar
...
\begin{table}[H]
  ...
  \begin{tabular}{lcccc}
    ...
    $d_\text{offset}$ {[}m{]}\textsuperscript{\S} & $3{,}86$ & $\bm{3{,}49}$ & $1{,}14$ & $\bm{1{,}08}$ \\
  ...
  \begin{minipage}{0.92\linewidth}\footnotesize
    ...
    \textsuperscript{\S}Los dos juegos se evalúan con el mismo cable al
    LO, $L_\text{LO} = 0{,}5\,\text{m}$, que no se midió con el VNA:
    también en la columna ``medido'' se le resta su retardo nominal
    ($2{,}53\,\text{ns}$). Es la única suposición que entra en esa
    columna.
  \end{minipage}
\end{table}

\textbf{El offset medido casi no depende de suponer nada}, y eso lo
hace el número más sólido de la tabla: sale de multiplicar el retardo
que se leyó en el rizado por $c_0/2$, sin pasar por el factor de
velocidad ni por el largo de cinta métrica de los cables de antena. Da
$3{,}49\,\text{m}$ contra los $3{,}86\,\text{m}$ que predice la
ecuación~\eqref{eq:d_offset}: un $10$\,\% menos.
