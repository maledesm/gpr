...
%  penetración terrestre (GPR) tipo FMCW/SFCW con resolución
%  mejorada mediante la transformada chirp-Z (CZT)
%============================================================
...
Desarrollar y validar un marco de procesamiento de señales basado en
la transformada chirp-Z para la estimación de alta resolución del
espesor de capas en sistemas GPR tipo FMCW/SFCW.
...
la FFT o la CZT, y los espectros resultantes son equivalentes.
...
\begin{enumerate}
  ...
  \item Opcionalmente, aplicar la CZT en una banda estrecha alrededor
    del pico para refinar la estimación de $\hat{f}^*$.
\end{enumerate}
...
$\Delta f = 4{,}00\,\text{Hz}$ medido con CZT:
...
%============================================================
\chapter{La Transformada Chirp-Z (CZT)}
\label{ch:czt}
%============================================================
...
($< 60\,\text{Hz}$). La CZT resuelve este problema sin modificar el
hardware.
...
\section{Descripción del entorno de simulación}

[Esta sección se completará con los resultados de simulación.
  Se describirán aquí los parámetros del sistema simulado, las señales
  generadas y las métricas de evaluación utilizadas (RMSE del espesor,
tasa de detección de picos, etc.).]

\section{Comparación FFT vs.\ CZT}

[Se incluirán figuras comparativas del espectro FFT y del zoom CZT
para distintos valores de espesor, así como tablas de error.]

\section{Sensibilidad a los parámetros}

[Análisis de sensibilidad respecto de $M$, del ancho del rango de zoom
y del valor de $\varepsilon_r$ asumido.]
...
\chapter{Derivación detallada de la señal IF}
\label{ap:derivacion}

A continuación se reproduce la derivación completa de la señal IF
para el caso FMCW, partiendo de la definición de la señal recibida.

\section{Señal transmitida}

Siguiendo la notación del apunte de cátedra, la señal transmitida es:

\begin{equation}
  s_\text{tx}(t) = A\cos\!\left(2\pi\frac{B}{\tau}\,t^2\right),
...
\begin{equation}
  f(t) = \frac{1}{2\pi}\frac{d}{dt}\!\left(2\pi\frac{B}{\tau}t^2\right) = \frac{2B}{\tau}\,t.
\end{equation}

Esta expresión confirma que la frecuencia crece linealmente con el
tiempo, con pendiente $2B/\tau$, como corresponde a un chirp lineal.

\section{Señal recibida y mezcla}

La señal recibida es una réplica retrasada $\tau_d = 2d/c$ segundos:

\begin{equation}
  \Gamma(t) = A^2\cos\!\left[2\pi\frac{B}{\tau}\!\left(t - \frac{2d}{c}\right)^{\!2} - \theta\right].
\end{equation}

Tras la multiplicación $s_\text{tx}(t)\cdot\Gamma(t)$ y el filtrado
pasa-bajos (véase Sección~\ref{sec:mixing}), se obtiene directamente
la ecuación~\eqref{eq:beat_freq}.

\chapter{Flujo completo del algoritmo}
\label{ap:flujo}

\begin{table}[H]
  \centering
  \caption{Resumen del flujo de procesamiento GPR FMCW con CZT.}
  \label{tab:flujo}
